import os, json, secrets, sqlite3, hashlib, zipfile, io, textwrap
...
def download_agent():
    """Genera e scarica l'agent già configurato per l'utente loggato"""
    if "user" not in session: return redirect("/")
    user = session["user"]
    pwd  = USERS[user]["password"]
    server_url = request.host_url.rstrip("/")

    # Genera il codice agent con credenziali già dentro
    agent_code = f'''"""
KAIRO Agent — avvia automaticamente con Windows
"""
import os, sys, subprocess, time, shutil, random, json, sqlite3
...
def heartbeat_loop():
    while True:
        try:
            payload = {{"tor_status": tor_status, "tor_ip": tor_ip, "active_sessions": list(chrome_procs.keys())}}
            r = requests.post(f"{{SERVER}}/api/agent/heartbeat", json=payload, headers=hdr(), timeout=10)
            d = r.json()
...
def handle_cmd(cmd):
    ...
    if action == "new_session": threading.Thread(target=start_session, args=(sid, cmd.get("label","Account")), daemon=True).start()
        ...
        s = sess_data.get(sid, {{}})
        open_chrome(sid, s.get("ua", random.choice(USER_AGENTS)), s.get("res", random.choice(RISOLUZIONI)), s.get("tz", random.choice(TIMEZONES)))
...
def scarica_tor():
    ...
    arch = os.path.join(TOR_DIR, "tor.tar.gz")
    try:
...
def avvia_tor():
    ...
    tor_status="online"; tor_ip="185.220.101."+str(random.randint(40,60))
    return True
...
def get_profile_dir(sid): return os.path.join(BASE_DIR, f"profile_{{sid}}")
...
def open_chrome(sid, ua, res, tz):
    ...
    os.makedirs(get_profile_dir(sid), exist_ok=True)
    flags = [chrome, f"--user-data-dir={{get_profile_dir(sid)}}",
        "--proxy-server=socks5://127.0.0.1:9050","--new-window",
        "--disable-webrtc","--webrtc-ip-handling-policy=disable_non_proxied_udp",
        "--disable-reading-from-canvas","--disable-webgl","--disable-webgl2",
        "--disable-blink-features=AutomationControlled","--disable-infobars",
        f"--user-agent={{ua}}",f"--window-size={{res}}",f"--timezone={{tz}}",
        "--lang=it-IT","--no-first-run","--disable-sync","--no-default-browser-check",
        "https://www.vinted.it/"]
    chrome_procs[sid] = subprocess.Popen(flags)
...
def leggi_cookie_chrome(sid):
    ...
    try:
        ...
        c={{r[0]:r[1] for r in rows if r[1]}}; return c or None
    except: return None
...
def start_session(sid, label):
    ...
    sess_data[sid]={{"ua":ua,"res":res,"tz":tz}}
    open_chrome(sid, ua, res, tz)
...
def read_session(sid):
    ...
    http_sess=crea_http(cookies)
    utente,email=get_vinted_info(http_sess)
    ...
    try:
        ...
        for c in r.json().get("conversations",[]):
            ...
            if cid and cid not in viste:
                viste.add(cid)
                nuove.append({{"offer_id":cid,"utente":c.get("opposite_user",{{}}).get("login","?"),
            ...
            if http:
                ...
                for o in fetch_offers(http, viste):
                    ...
                    try:
                        ...
                        try:
                            import ctypes
                            ctypes.windll.user32.MessageBoxW(0,f"Da: {{o[\'utente\']}}\\n{{o[\'msg\']}}","Nuova offerta Vinted!",0x40)
                        except: pass
                    except: pass
        except: pass
        stop_ev.wait(30)
...
'''
    ...
    # Crea un .bat che installa requests e avvia l'agent invisibile
    bat_code = f'''@echo off
pip install requests --quiet --disable-pip-version-check >nul 2>&1
start "" /B pythonw kairo_agent.py
exit
'''

    # Metti tutto in uno zip in memoria
    buf = io.BytesIO()
    with zipfile.ZipFile(buf, 'w', zipfile.ZIP_DEFLATED) as z:
        z.writestr("kairo_agent.py", agent_code)
        z.writestr("avvia.bat", bat_code)
    buf.seek(0)
    ...
    return send_file(
        ...
        as_attachment=True,
        download_name=f'kairo_agent_{user}.zip'
    )
